...
export default function RegisterPage() {
  ...
  const { registerWithEmail, signInWithGoogle, signInWithGithub } = useAuth();
  ...
  return (
    <AuthLayout
      eyebrow="Get Started"
      title="Join CollabHub"
      lead="Connect with innovators and build together."
      wide={true}
      footer={
        <span>
    ...
    >
      {error && (
        <div className="flex items-center gap-3 p-4 mb-4 rounded-xl bg-destructive/10 text-destructive text-sm font-semibold border border-destructive/20">
          <AlertCircle size={16} />
          <span>{error}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <Button
          ...
          className="w-full flex items-center justify-center bg-white dark:bg-zinc-800 text-foreground hover:bg-zinc-50 dark:hover:bg-zinc-700 border-border/50 shadow-sm transition-all h-11 rounded-xl"
        >
          <GoogleIcon />
      ...
      <div className="relative flex py-3 items-center justify-center mb-5">
        <div className="absolute inset-x-0 top-1/2 border-t border-border/50 -z-10" />
        <span className="bg-white dark:bg-zinc-900 px-4 relative z-10 text-[10px] font-bold uppercase tracking-widest text-muted-foreground rounded-full shadow-sm border border-border/50">
